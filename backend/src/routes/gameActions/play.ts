...
export const moveToBattlefield: ActionMethod<MoveToBattlefieldMetadata> = async (_gameId, seat, metadata) => {
	const objectId = metadata.objectId;
	const position = metadata.position;
	if (objectId) {
		let updateQuery = `UPDATE game_objects SET zone = 'battlefield', controller = $2`;
		const params: any[] = [objectId, seat];
...
export const cast: ActionMethod = async (_gameId, seat, metadata) => {
	// Move card from command zone to battlefield (put into play)
	const objectId = metadata.objectId;
	if (objectId) {
		const position = metadata.position || { x: 100, y: 100 };
		await query(`UPDATE game_objects SET zone = 'battlefield', position = $1, controller=$2  WHERE id = $3`, [
