...
export const addCounter: ActionMethod = async (
	gameId,
	seat,
	metadata: Metadata,
) => {
	...
	const result = await query<{ counters: Record<string, number> | null }>(
		`SELECT counters FROM game_objects WHERE id = $1`,
		[objectId],
	);
	let counters =
		(result?.rows?.[0]?.counters as Record<string, number> | null) || {};
...
export const removeCounter: ActionMethod = async (
	gameId,
	seat,
	metadata: Metadata,
) => {
